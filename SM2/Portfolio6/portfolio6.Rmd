...
This is further backed up by the fact that this model achieves a prediction accuracy of 68.7% on the test set.
```{r}
yTestPrediction = predict(model, XTest) > 0
mean(yTest == yTestPrediction)
```

Compare this to a generalised linear model, which only achieves a prediction accuracy of 65.7% on the test set. 
This is slightly lower than the previous model, suggesting that the nonlinearity captures useful behaviour within the dataset.
<!-- # ```{r} -->
