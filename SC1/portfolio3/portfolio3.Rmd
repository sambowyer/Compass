...
For large tasks we can distribute computation across multiple CPU cores in the hopes of a speed increase (though this won't help if the task is slow for a non-CPU-related bottleneck,m such as within the task's use of memory, networking or I/O).
...
Consider the function below which squares the square-root of each entry in a matrix:
```{r}
id <- function(X){
  # return (apply(X, c(1,2), function(x) sqrt(x)**2))
  #X_new = matrix(NA, nrow=nrow(X), ncol=ncol(X))
  #for(i in 1:nrow(X)){
  #  for(j in 1:ncol(X)){
  #    X_new[i,j] = sqrt(X[i,j])**2
  #  }
  #}
  #return(X_new)
  return(sqrt(X)**2)
}
X = matrix(1:1e7, nrow=500, ncol=20000)
...
Note that it is not necessarily true that using $n$ cores will result in an $n$-factor speed-up, in fact in general the gains tend to diminish as $n$ grows larger, and using too many cores may actually slow you down due to the time required for the core-allocation of computations to take place.
...
X = matrix(1:1e6, nrow=500, ncol=2000)
```
And then we can compare the performance of `%do%` and `%dopar%` as follows:
```{r}
system.time(foreach (i=1:100) %do% {id(X)})
```
```{r}
system.time(foreach (i=1:100) %dopar% {id(X)})
```
